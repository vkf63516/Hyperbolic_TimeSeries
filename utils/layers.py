...
def normalize_decomposed_tensors(train_tensors_dict, val_tensors_dict, test_tensors_dict):
    scaler = StandardScaler()
    # concatenate all training decomposed parts
    train_concat = []
    for feat, comps in train_tensors_dict.items():
        for key in ["trend", "seasonal", "residual"]:
            x = comps[key].squeeze(0).cpu().numpy()  # [T,C]
            train_concat.append(x)
    train_concat = np.concatenate(train_concat, axis=0)
    scaler.fit(train_concat)

    def apply_scaler(tensors_dict):
        out = {}
        for feat, comps in tensors_dict.items():
            out[feat] = {}
            for key, t in comps.items():
                x = t.squeeze(0).cpu().numpy()
                x_scaled = scaler.transform(x)
                out[feat][key] = torch.tensor(x_scaled).unsqueeze(0)
        return out

    train_scaled = apply_scaler(train_tensors_dict)
    val_scaled   = apply_scaler(val_tensors_dict)
    test_scaled  = apply_scaler(test_tensors_dict)
