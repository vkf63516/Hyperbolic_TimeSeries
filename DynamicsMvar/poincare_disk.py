...
class PoincareBallStdGrad(nn.Module):
    ...
    def mobius_scalar_mul(self, r: torch.Tensor, x: torch.Tensor, dim: int = -1):
        return dg_mobius_scalar_mul(r=r, x=x, c=self.c, dim=dim)
...
class PoincareBallCustomAutograd(nn.Module):
    ...
    def mobius_scalar_mul(
        self, r: torch.Tensor, x: torch.Tensor, dim: int = -1
    ) -> torch.Tensor:
        return ag_MobiusScalarMul.apply(r, x, self.c, dim)
