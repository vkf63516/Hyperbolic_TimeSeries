...
class ag_MobiusScalarMul(torch.autograd.Function):
    @staticmethod
    def forward(ctx, r, x, c, dim=-1, eps=1e-15):
        x_norm = x.norm(dim=dim, keepdim=True).clamp_min(eps)
        sqrt_c = c.sqrt()

        u = x_norm * sqrt_c
        atanh_u = torch.atanh(u)
        t = r * atanh_u
        tanh_t = torch.tanh(t)

        res = (tanh_t / sqrt_c) * (x / x_norm)

        ctx.save_for_backward(r, x, c, x_norm, sqrt_c, u, atanh_u, t, tanh_t)
        ctx.dim = dim
        ctx.eps = eps
        return res
    ...
    @staticmethod
    def backward(ctx, grad_output):
        r, x, c, x_norm, sqrt_c, u, atanh_u, t, tanh_t = ctx.saved_tensors
        dim = ctx.dim
        eps = ctx.eps

        # Common terms
        sech2_t = 1 - tanh_t ** 2
        inv_x_norm = 1.0 / x_norm
        x_hat = x * inv_x_norm

        # ---- grad w.r.t. r ----
        grad_r = grad_output * (atanh_u * sech2_t / sqrt_c) * x_hat
        grad_r = grad_r.sum(dim=dim, keepdim=False)

        # ---- grad w.r.t. x ----
        du_dx = sqrt_c * x_hat
        datanh_du = 1.0 / (1 - u ** 2)
        dt_dx = r * datanh_du * du_dx

        dscalar_dx = (sech2_t * dt_dx) / sqrt_c

        # Efficient projection form
        proj = (grad_output * x_hat).sum(dim=dim, keepdim=True)
        grad_x = (tanh_t / sqrt_c) * (
            grad_output * inv_x_norm - proj * x * inv_x_norm ** 2
        ) + proj * dscalar_dx * x_hat

        # ---- grad w.r.t. c ----
        dsqrtc_dc = 0.5 / sqrt_c
        du_dc = x_norm * dsqrtc_dc
        datanh_dc = datanh_du * du_dc
        dt_dc = r * datanh_dc

        dscalar_dc = (
            sech2_t * dt_dc / sqrt_c
            - 0.5 * tanh_t / (c * sqrt_c)
        )

        grad_c = (grad_output * x_hat * dscalar_dc).sum(dim=dim, keepdim=False)

        return grad_r, grad_x, grad_c, None
