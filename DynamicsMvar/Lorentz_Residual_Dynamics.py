...
def lorentzian_residual_update(x_current, x_update, manifold, alpha=0.7, eps=1e-8, c=1.0):
    ...
    numerator = alpha * x_current + (1 - alpha) * x_update
    
    # Compute Lorentzian norm of weighted sum
    lorentz_norm = lorentzian_norm(numerator, manifold, eps=eps)
    
    # Get curvature (for Lorentz, k is typically 1.0)
    c = torch.abs(torch.tensor(manifold.k, device=x_current.device))
    sqrt_c = torch.sqrt(c)
    
    # Normalize: x_next = numerator / (sqrt(c) * ||numerator||_L)
    denominator = sqrt_c * lorentz_norm
    x_next = numerator / denominator.unsqueeze(-1)
    
    # Project back to manifold for numerical stability
...
class HyperbolicLorentzDynamics(nn.Module):
    ...
    def forward(self, x_current):
        ...
        x_update = safe_expmap0(self.manifold, velocity)  # [B, embed_dim+1]
