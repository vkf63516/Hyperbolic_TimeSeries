...
class ParallelPoincare(nn.Module):
    def __init__(self, lookback, input_dim, embed_dim=32, hidden_dim=64, 
        ...
        self.weekly_embed = MambaEmbed(input_dim, hidden_dim, embed_dim, lookback=lookback, n_layer=n_layer)  # hourly, daily, weekly
        self.daily_embed = MambaEmbed(input_dim, hidden_dim, embed_dim, lookback=lookback, n_layer=n_layer)
    ...
    def forward(self, trend, seasonal_weekly, seasonal_daily, residual):
        ...
        z_combined = self.manifold.mobius_add(self.manifold.mobius_add(z_trend_h,
                                             self.manifold.mobius_add(z_weekly_h, z_daily_h)),
                                             z_residual_h)
